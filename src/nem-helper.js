...
let nemhelper = {
    ...
    updateFee: (state) => {
		...
		var feeString = parseInt(nem.utils.format.nemValue(transactionEntity.fee)[0].replace(/\s+/g, "")) + "." + nem.utils.format.nemValue(transactionEntity.fee)[1];
    ...
    signTransaction: (state) => {
		...
		if(!prv || !address) return alert('Missing parameter !');
		if(undefined === amount|| !nem.utils.helpers.isTextAmountValid(amount)) return alert('Invalid amount !');
